...
$coreDllSources = @{
    ...
    "NOLoader.Telemetry.dll" = Join-Path $RepoRoot "src\NOLoader.Telemetry\bin\$Configuration\net48\NOLoader.Telemetry.dll"
...
foreach ($dll in $coreDlls) {
    ...
    if (-not (Test-Path $srcPath)) {
        Write-Error "Missing $dll for $Configuration"
    }
